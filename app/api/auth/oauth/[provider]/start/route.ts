...
export async function GET(request: NextRequest, { params }: { params: Promise<{ provider: string }> }) {
  ...
  if (!isKnownProvider(provider)) {
    return NextResponse.json({ error: { code: "UNKNOWN_PROVIDER", message: (await getTranslations("ApiErrors"))("unknownProvider") } }, { status: 404 });
  }
  ...
  if (limited.limited) {
    return NextResponse.json({ error: { code: "RATE_LIMITED", message: (await getTranslations("ApiErrors"))("tooManyAttempts") } }, { status: 429 });
  }
  await recordAttempt(key);

  const client = getOAuthProvider(provider);
  if (!client) {
    return NextResponse.json({ error: { code: "PROVIDER_NOT_CONFIGURED", message: (await getTranslations("ApiErrors"))("providerNotConfigured") } }, { status: 404 });
  }
